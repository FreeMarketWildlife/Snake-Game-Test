...
let ghostMix=0,ghostPan=0,lastSpiritDeathSlot=-1,spiritCues=0;
...
function schedule(){if(!ready||ac.state!=='running')return;while(next<ac.currentTime+1){playBar(bar,next);playGhostBar(bar,next);next+=barLen;bar=(bar+1)%SONG.length}}
