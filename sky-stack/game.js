...
const removedTerrain=new Set(),terrainDamage=new Map();
...
let pendingPlacement=null,restoring=false,resetting=false,lastSavedAt=0;
...
function terrainMaterial(cx,cy){
  ...
  return hash01(cx,cy)<step/(TRANSITION+1)?'stone':'dirt'
}
...
if(initialSave){
  best=Math.max(0,finite(initialSave.best));
  shape=best>=5;stone=best>=10;
...
function mk(px,py,m,o={}){
  ...
  if(o.velocity)Body.setVelocity(z,{x:finite(o.velocity.x),y:finite(o.velocity.y)});
  if(o.angularVelocity)Body.setAngularVelocity(z,finite(o.angularVelocity));
  bs.add(z);
...
function gen(ch){
  ...
  for(let cy=0;cy<DEPTH;cy++)for(let cx=ch*24;cx<ch*24+24;cx++){
    ...
    const m=terrainMaterial(cx,cy),hits=terrainDamage.get(k)||0;
    mk(ctr(cx),ctr(cy),m,{static:true,terrain:true,cx,cy,hits})
  }
}
...
function createMinerAt(x,y,o={}){
  const q=Bodies.rectangle(x,y,B*.66,B*.80,{
    density:.00125,friction:.88,frictionStatic:.95,restitution:.02,frictionAir:.025,
    chamfer:{radius:2},sleepThreshold:100
  });
  const id=Math.max(1,Math.floor(o.id||++minerId));
  minerId=Math.max(minerId,id);
  q.game={kind:'miner',level:1,id,dir:o.dir===-1?-1:1,nextMine:performance.now()+700+Math.random()*300,nextTurn:performance.now()+800+Math.random()*1200,pickUntil:0};
  if(o.angle)Body.setAngle(q,o.angle);
  if(o.velocity)Body.setVelocity(q,{x:finite(o.velocity.x),y:finite(o.velocity.y)});
  if(o.angularVelocity)Body.setAngularVelocity(q,finite(o.angularVelocity));
  miners.add(q);World.add(eng.world,q);return q
}

function ensureChunkAtX(x){
  const ch=Math.floor(x/(24*B));
  for(let i=ch-1;i<=ch+1;i++)gen(i)
}

function restoreDynamicState(s){
  if(!s)return;
  restoring=true;
  if(Array.isArray(s.placed))for(const p of s.placed){
    if(!M[p.material])continue;
    ensureChunkAtX(finite(p.x));
    mk(finite(p.x),finite(p.y),p.material,{placed:true,w:Math.max(4,finite(p.w)||B-1),h:Math.max(4,finite(p.h)||B-1),cost:Math.max(1,Math.floor(finite(p.cost)||1)),hits:Math.max(0,Math.floor(finite(p.hits))),angle:finite(p.angle),velocity:{x:finite(p.vx),y:finite(p.vy)},angularVelocity:finite(p.av)})
  }
  if(Array.isArray(s.miners))for(const m of s.miners){ensureChunkAtX(finite(m.x));createMinerAt(finite(m.x),finite(m.y),{id:m.id,dir:m.dir,angle:finite(m.angle),velocity:{x:finite(m.vx),y:finite(m.vy)},angularVelocity:finite(m.av)})}
  restoring=false
}
...
function saveGame(){
  ...
  try{
    const placed=[];
    for(const z of bs)if(z.game?.placed)placed.push({material:z.game.material,x:z.position.x,y:z.position.y,w:z.game.w,h:z.game.h,cost:z.game.cost,hits:z.game.hits||0,angle:z.angle,vx:z.velocity.x,vy:z.velocity.y,av:z.angularVelocity});
    const workerData=[];
    for(const q of miners)workerData.push({id:q.game.id,dir:q.game.dir,x:q.position.x,y:q.position.y,angle:q.angle,vx:q.velocity.x,vy:q.velocity.y,av:q.angularVelocity});
    const s={v:1,savedAt:Date.now(),best,inv:{dirt:inv.dirt,stone:inv.stone},removed:[...removedTerrain],damage:[...terrainDamage.entries()],placed,miners:workerData,cam:{x:cam.x,y:cam.y}};
...
function saveSoon(){
  if(restoring)return;
  if(performance.now()-lastSavedAt>350)saveGame()
}
...
function toast(t,long=false){
  clearTimeout(toastTimer);$('toast').textContent=t;$('toast').classList.remove('hide');
...
function ui(){
  $('dc').textContent=inv.dirt;$('sc').textContent=inv.stone;
  ...
  $('mc').textContent=available;$('minerSlot').classList.toggle('unlocked',total>0);$('minerSlot').classList.toggle('locked',total===0);$('minerSlot').classList.toggle('empty',total>0&&available===0);
...
function remove(z){
  ...
  if(z.game.terrain){const k=key(z.game.cx,z.game.cy);grid.delete(k);removedTerrain.add(k);terrainDamage.delete(k)}
  bs.delete(z);World.remove(eng.world,z);wake();ui();saveSoon()
}

function mine(p){
  const z=at(p);if(!z)return;
  if(z.game.material==='stone'&&!stone){toast('🔒 STONE LOCKED\nReach 10 m first!',true);return}
  z.game.hits++;z.game.flash=7;
  if(z.game.terrain){const k=key(z.game.cx,z.game.cy);terrainDamage.set(k,z.game.hits)}
  const broken=z.game.hits>=z.game.max;SkyAudio.hit(z.game.material,broken,z.game.hits);
  if(broken)remove(z);else saveSoon()
}

function rect(a,b,limit=Infinity){
  const ax=cell(a.x),ay=cell(a.y);let bx=cell(b.x),by=cell(b.y);
  if(Number.isFinite(limit)){bx=ax+clamp(bx-ax,-(limit-1),limit-1);by=ay+clamp(by-ay,-(limit-1),limit-1)}
...
$('overlapPlace').addEventListener('click',e=>{
  e.stopPropagation();
  if($('overlapDont').checked){overlapSuppressed=true;try{localStorage.setItem('skyStack.hideOverlapWarning','1')}catch{}}
  const commit=pendingPlacement;pendingPlacement=null;hideOverlapWarning();SkyAudio.ui();if(commit)commit()
});
$('overlapCancel').addEventListener('click',e=>{e.stopPropagation();pendingPlacement=null;hideOverlapWarning();SkyAudio.ui()});
...
function one(m,p){
  ...
  attemptPlacement(r,()=>{if(!inv[m])return toast('NO '+m.toUpperCase()+' — MINE SOME!');inv[m]--;mk(ctr(r.minx),ctr(r.miny),m,{placed:true});ui();SkyAudio.place(m,1);saveSoon()})
}

function solid(m,a,b){
  const r=rect(a,b,buildLimit());if(r.cost===1)return one(m,b);if(inv[m]<r.cost)return toast('NEED '+r.cost+' '+m.toUpperCase());
  attemptPlacement(r,()=>{if(inv[m]<r.cost)return toast('NEED '+r.cost+' '+m.toUpperCase());inv[m]-=r.cost;mk((r.minx+r.maxx+1)*B/2,(r.miny+r.maxy+1)*B/2,m,{w:r.nx*B-1,h:r.ny*B-1,cost:r.cost,placed:true});ui();SkyAudio.place(m,r.cost);saveSoon()})
}

function minerDropPoint(p){
  const halfW=B*.34,halfH=B*.42;let y=p.y;
  for(let i=0;i<10;i++){const hits=Query.region([...bs],{min:{x:p.x-halfW,y:y-halfH},max:{x:p.x+halfW,y:y+halfH}});if(!hits.length)return{x:p.x,y};y-=B}
  return{x:p.x,y}
}
...
function placeMiner(p){
  ...
  const drop=minerDropPoint(p);createMinerAt(drop.x,drop.y,{dir:Math.random()<.5?-1:1});ui();SkyAudio.ui();toast('LEVEL 1 MINER DEPLOYED ⛏');saveSoon()
}

function mineableNear(q){const r=B*1.18;return Query.region([...bs],{min:{x:q.position.x-r,y:q.position.y-r},max:{x:q.position.x+r,y:q.position.y+r}}).filter(z=>z.game?.terrain&&z.game.material==='dirt')}
function updateMiners(now){
  for(const q of miners){
    ...
    if(now>=g.nextMine){
      ...
      if(targets.length){const target=targets[Math.floor(Math.random()*targets.length)],dx=target.position.x-q.position.x;if(Math.abs(dx)>4)g.dir=Math.sign(dx);g.pickUntil=now+240;remove(target)}
...
function openMinerMenu(q){if(!miners.has(q))return;selectedMiner=q;$('minerMenu').classList.add('show');$('minerMenu').setAttribute('aria-hidden','false');SkyAudio.ui()}
function closeMinerMenu(){selectedMiner=null;$('minerMenu').classList.remove('show');$('minerMenu').setAttribute('aria-hidden','true')}
$('minerBreak').addEventListener('click',e=>{e.stopPropagation();if(selectedMiner&&miners.has(selectedMiner)){World.remove(eng.world,selectedMiner);miners.delete(selectedMiner);toast('MINER IS TAKING A BREAK ☕');ui();saveSoon()}closeMinerMenu();SkyAudio.ui()});
...
$('bar').addEventListener('click',e=>{
  ...
  if(t==='miner'){if(unlockedMiners()<1)return toast('🔒 LEVEL 1 MINER UNLOCKS AT 20 m',true);if(availableMiners()<1)return toast('ALL YOUR MINERS ARE WORKING\nTap one to give him a break.',true)}
...
$('restartConfirm').addEventListener('click',e=>{
  e.stopPropagation();if(!$('restartAgree').checked)return;
  resetting=true;
...
function end(e){
  e.preventDefault();if(!gesture||gesture.id!==e.pointerId)return;const g=gesture;gesture=null;
  if(!g.moved){const worker=minerAt(g.b);if(worker){openMinerMenu(worker);return}}
  if(g.t==='move')return;
  if(g.t==='pick'){if(!g.moved)mine(g.b);return}
  if(g.t==='miner'){if(!g.moved)placeMiner(g.b);return}
  if(!g.moved)one(g.t,g.b);else if(shape)solid(g.t,g.a,g.b)
}
...
$('top').onclick=()=>{const p=topPoint();go(p||{x:cam.x,y:-16});SkyAudio.ui()};
...
function quest(){
  ...
  const goals=[],lim=buildLimit();
  if(lim<10){const h=5+(lim-1)*10;goals.push({h,title:'Reach '+h+' m',sub:'Unlock '+(lim+1)+'×'+(lim+1)+' building'})}
  const mh=nextMinerMilestone();goals.push({h:mh,title:'Reach '+mh+' m',sub:'Unlock +1 Level 1 miner'});goals.sort((a,b)=>a.h-b.h);
  $('questTitle').textContent=goals[0].title;$('questSub').textContent=goals[0].sub
}
...
function progress(){
  ...
  if(m>best){
    ...
    const gained=minerCountAt(best)-oldMinerCount;if(gained>0){toast('★ MINER UNLOCKED! ★\n+'+gained+' LEVEL 1 MINER'+(gained>1?'S':'')+' ⛏',true);SkyAudio.unlock()}
    ui();quest();saveSoon()
  }
}
...
function bg(){
  ...
  const groundY=w2s(0,0).y,deepY=w2s(0,DEPTH*B).y;
  if(groundY<HH){const earth=ctx.createLinearGradient(0,groundY,0,Math.max(groundY+1,deepY));earth.addColorStop(0,'#4c3020');earth.addColorStop(.16,'#422b1e');earth.addColorStop(.30,'#3a302a');earth.addColorStop(.43,'#303034');earth.addColorStop(.62,'#272b31');earth.addColorStop(1,'#171b22');ctx.fillStyle=earth;ctx.fillRect(0,Math.max(0,groundY),W,HH-Math.max(0,groundY))}
  ctx.fillStyle='#72b552';ctx.fillRect(0,groundY-5*cam.z,W,6*cam.z)
}

function cracks(z,sw,sh){
  if(z.game.max<=1||z.game.hits<=0)return;const n=Math.ceil(z.game.hits/z.game.max*4),paths=[[[-.1,-.5],[-.02,-.2],[-.18,0],[.03,.15]],[[-.5,.05],[-.25,0],[-.1,.2],[-.22,.42]],[[.5,-.2],[.25,-.1],[.1,.08],[.3,.25]],[[.1,.5],[.05,.25],[-.08,.1],[.08,-.05]]];ctx.strokeStyle='#222b35';ctx.lineWidth=Math.max(1,1.5*cam.z);for(let i=0;i<n;i++){ctx.beginPath();paths[i].forEach((p,j)=>j?ctx.lineTo(p[0]*sw,p[1]*sh):ctx.moveTo(p[0]*sw,p[1]*sh));ctx.stroke()}
}

function draw(z){
  const s=w2s(z.position.x,z.position.y),sw=z.game.w*cam.z,sh=z.game.h*cam.z;if(s.x+sw/2<0||s.x-sw/2>W||s.y+sh/2<0||s.y-sh/2>HH)return;
  ctx.save();ctx.translate(s.x,s.y);ctx.rotate(z.angle);ctx.fillStyle=M[z.game.material].col[0];ctx.fillRect(-sw/2,-sh/2,sw,sh);ctx.fillStyle=M[z.game.material].col[1];ctx.fillRect(-sw/2,sh/2-Math.max(2,3*cam.z),sw,Math.max(2,3*cam.z));cracks(z,sw,sh);if(z.game.flash>0){ctx.strokeStyle='#fff3a0';ctx.lineWidth=3;ctx.strokeRect(-sw/2+2,-sh/2+2,sw-4,sh-4);z.game.flash--}ctx.restore()
}
...
function drawMiner(q,now){
  ...
  ctx.save();ctx.translate(s.x,s.y);ctx.rotate(q.angle);ctx.scale(flip,1);ctx.lineWidth=Math.max(1,2*cam.z);ctx.strokeStyle='#243247';ctx.fillStyle='#26384b';ctx.fillRect(-w*.30,h*.27,w*.22,h*.18);ctx.fillRect(w*.08,h*.27,w*.22,h*.18);ctx.fillStyle='#3c75a6';ctx.fillRect(-w*.34,-h*.05,w*.68,h*.36);ctx.strokeRect(-w*.34,-h*.05,w*.68,h*.36);ctx.fillStyle='#f0b78d';ctx.fillRect(-w*.29,-h*.35,w*.58,h*.34);ctx.strokeRect(-w*.29,-h*.35,w*.58,h*.34);ctx.fillStyle='#f0c84c';ctx.fillRect(-w*.36,-h*.46,w*.72,h*.14);ctx.fillRect(-w*.27,-h*.54,w*.54,h*.11);ctx.strokeRect(-w*.36,-h*.46,w*.72,h*.14);ctx.fillStyle='#243247';const eye=Math.max(1,2.1*cam.z);ctx.fillRect(w*.02,-h*.22,eye,eye);ctx.fillRect(w*.18,-h*.22,eye,eye);
...
function loop(now){
  Engine.update(eng,16.67);updateMiners(now||performance.now());
  ...
  ensureWorld();progress();ctx.setTransform(DPR,0,0,DPR,0,0);bg();for(const z of bs)draw(z);for(const q of miners)drawMiner(q,now||performance.now());preview();requestAnimationFrame(loop)
}
